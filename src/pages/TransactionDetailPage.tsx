  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      <div className="space-y-3">
        ...
        {isSupplier && ['PAYMENT_CONFIRMED', 'LOGISTICS_PENDING', 'LOGISTICS_ASSIGNED', 'LOGISTICS_ACCEPTED', 'READY_FOR_PICKUP', 'PICKED_UP', 'IN_TRANSIT', 'DELIVERED', 'BUYER_CONFIRMATION_PENDING'].includes(txn.status) && (
          <div className="px-4 py-4 bg-emerald-50 border border-emerald-200 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-start gap-3">
              <div className="p-2 bg-emerald-100 text-emerald-800 rounded-lg shrink-0 mt-0.5">
                <CheckCircle2 className="w-5 h-5 text-emerald-600" />
              </div>
              <div>
                <p className="text-sm font-semibold text-emerald-950">Funds Secured in Escrow</p>
                <p className="text-xs text-emerald-800 mt-0.5">
                  The buyer has funded the escrow ({formatCurrency(txn.totalAmount, txn.currency)}). Funds are safely held in AgriFlow Escrow and will be automatically released to your available payout balance upon delivery confirmation.
                </p>
              </div>
            </div>
            <div className="text-right shrink-0">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-900 border border-emerald-300 shadow-xs">
                🔒 {formatCurrency(txn.totalAmount, txn.currency)} In Escrow
              </span>
            </div>
          </div>
        )}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* Payment */}
          {(payment || ['PAYMENT_CONFIRMED', 'LOGISTICS_PENDING', 'LOGISTICS_ASSIGNED', 'LOGISTICS_ACCEPTED', 'READY_FOR_PICKUP', 'PICKED_UP', 'IN_TRANSIT', 'DELIVERED', 'BUYER_CONFIRMATION_PENDING', 'COMPLETED'].includes(txn.status)) && (
            <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-xs">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Escrow & Payment</h2>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 border border-emerald-200 text-emerald-800">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                  Escrow Protected
                </span>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <div><div className="text-xs text-gray-500 mb-0.5">Payment ID</div><div className="text-sm font-mono text-gray-700">{payment?.id || txn.paymentId || 'PAY-AGF-ESCROW'}</div></div>
                <div><div className="text-xs text-gray-500 mb-0.5">Escrow Amount</div><div className="text-sm font-bold text-gray-900">{formatCurrency(payment?.amount || txn.totalAmount, txn.currency)}</div></div>
                <div><div className="text-xs text-gray-500 mb-0.5">Provider</div><div className="text-sm text-gray-700">{payment?.provider || 'AgriFlow Escrow Settlement'}</div></div>
                <div>
                  <div className="text-xs text-gray-500 mb-0.5">Status</div>
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full border inline-block
                    ${(payment?.status === 'CONFIRMED' || ['PAYMENT_CONFIRMED', 'LOGISTICS_PENDING', 'LOGISTICS_ASSIGNED', 'LOGISTICS_ACCEPTED', 'READY_FOR_PICKUP', 'PICKED_UP', 'IN_TRANSIT', 'DELIVERED', 'BUYER_CONFIRMATION_PENDING', 'COMPLETED'].includes(txn.status)) ? 'bg-green-50 border-green-200 text-green-700' :
                      payment?.status === 'FAILED' ? 'bg-red-50 border-red-200 text-red-700' :
                      'bg-amber-50 border-amber-200 text-amber-700'}`}>
                    {payment?.status || (['PAYMENT_CONFIRMED', 'LOGISTICS_PENDING', 'LOGISTICS_ASSIGNED', 'LOGISTICS_ACCEPTED', 'READY_FOR_PICKUP', 'PICKED_UP', 'IN_TRANSIT', 'DELIVERED', 'BUYER_CONFIRMATION_PENDING', 'COMPLETED'].includes(txn.status) ? 'CONFIRMED (ESCROW LOCKED)' : 'PENDING')}
                  </span>
                </div>
                {(payment?.providerReference || txn.paymentId) && (
                  <div className="sm:col-span-2">
                    <div className="text-xs text-gray-500 mb-0.5">Provider Reference</div>
                    <div className="text-sm font-mono text-gray-700">{payment?.providerReference || txn.paymentId}</div>
                  </div>
                )}
                {payment?.completedAt && (
